...
var pf = {
    entradas: document.querySelectorAll("input.obligatorio"),
    valor: null,
    validarUsuario: true,
...
var mf = {
    ...
    enFoco: function(input){
        ...
        /*console.log(input.target.value); para llegar al valor del input*/
        pf.valor = input.target.value;
        if(pf.valor == ""){

            document.querySelector("#"+input.target.id).style.background = "rgba(255,255,0,.5)";
            document.querySelector("[for="+input.target.id+"] .mensajeObligatorio").style.opacity = 1;
    ...
    fueraFoco: function(input){
        ...
        document.querySelector("[for="+input.target.id+"] .mensajeObligatorio").style.opacity = 0;
    ...
    validarFormulario: function(){
        ...
        if(!pf.validarUsuario || !pf.validarPassword || !pf.validarEmail){
            ...
            document.querySelector("#labelEnviar").innerHTML = '<span style="color:red">*Campos obligatorios incompletos</span>'
            return false;
